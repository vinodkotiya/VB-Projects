...
Begin VB.Form frmAlbum 
   ...
   Begin TabDlg.SSTab SSTab1 
      ...
      ForeColor       =   16711680
      TabCaption(0)   =   "&Make"
      TabPicture(0)   =   "linkexe.frx":1CCA
      Tab(0).ControlEnabled=   -1  'True
      Tab(0).Control(0)=   "Frame4"
      Tab(0).Control(0).Enabled=   0   'False
      Tab(0).ControlCount=   1
      TabCaption(1)   =   "&Settings"
      TabPicture(1)   =   "linkexe.frx":1CE6
...
On Error GoTo fileerror
    ...
    Line Input #FNum, currentline '<no_of_images>
       Line Input #FNum, currentline
    ...
    Line Input #FNum, currentline '<alt>
       ...
       Line Input #FNum, currentline 'something value below
        ...
        txtAlt.Enabled = True
        End If
   Line Input #FNum, currentline '<imageper page>
       Line Input #FNum, currentline
...
Dim vinod As Boolean
   ...
   Dim FNum As Integer
   FNum = FreeFile
   Open firstFolder & "\" & "start.html" For Output As FNum
...
For i = 0 To File1.ListCount - 1                 'scanning current folder
  ...
  'Text1.Text = Text1.Text & vbCrLf & File1.List(i)
     ...
     Label(1).Caption = "File no : " & i & "   Total Files Scanned : " & totalFiles + i + 1 & "   Time : " & DateDiff("s", starttime, Now) & " Seconds  "
      If optAlt(1).Value = True Then altText = File1.List(i)        'if user not give alternate text then use filename
       Set ThisFile = Fsys.GetFile(File1.Path & "\" & File1.List(i))             ' for file attributes
           ...
           '-----------------------making single cell for each  file
           txtScanned.Text = txtScanned.Text & "<td> <center><a href = " & Chr(34) & Replace(Right(File1.Path, Len(File1.Path) - Len(firstFolder) - 1), "\", "/") & "/" & File1.List(i) & Chr(34) & "> <FONT size = 4>" & File1.List(i) & "</FONT></a><font size = 2 color =" & Chr(34) & hcodeTEXT & Chr(34) & ">"
            
            If chkInfo(0).Value Then txtScanned.Text = txtScanned.Text & "<BR>Type " & ThisFile.Type
        ...
        If (i + 1) Mod imagesinrow = 0 Then     'when 1 row com[plete switch to next row
              txtScanned.Text = txtScanned.Text & vbCrLf & "</tr>" & vbCrLf & vbCrLf & "<tr>"
       End If
...
Dim v As Integer
Upper ("")
txtScanned.Text = txtScanned.Text & "<br><br><center> <font size = 7><b><u> WebAlbum Home Page</u></b> </font></center><br><br>" & vbCrLf
...
Dim v As Integer
 txtScanned.Text = "[autorun]" & vbCrLf & "open = start start.html" & vbCrLf & "icon = vin.ico"
v = FreeFile
